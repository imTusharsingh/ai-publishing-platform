    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-surface shadow-card transition-shadow hover:shadow-panel">
      ...
      <div className="flex flex-1 flex-col p-5">
        <Link
          ...
          className="inline-flex w-fit rounded-full bg-surface-muted px-2.5 py-1 text-xs font-medium text-content-muted transition-colors hover:text-content"
        >
          {article.category.name}
        </Link>
        <Link href={`/articles/${article.slug}`} className="mt-3 flex flex-1 flex-col">
          <h3 className="text-lg font-semibold leading-snug text-content transition-colors group-hover:text-content/80">
            {article.title}
          </h3>
